...
"""
class PMA(nn.Module):
    def __init__(self, k_seeds, head_num, d_model, use_layer_norm = True, split = False):
        super().__init__()
        ...
        self.mab = MAB(head_num, d_model, d_model, d_model, use_layer_norm, split)
        self.sab = None
        if k_seeds > 1:
            self.sab = SAB(head_num, d_model, d_model, use_layer_norm, split)
        """

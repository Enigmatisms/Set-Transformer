...
class MAB(nn.Module):
    def __init__(self, head_num, dim_q, dim_k, dim_v, use_layer_norm = True):
        super().__init__()
        ...
        self.att = Multihead(head_num, dim_v, dim_v)
...
class SAB(nn.Module):
    def __init__(self, head_num, dim_in, dim_out, use_layer_norm = True):
        super().__init__()
        self.mab = MAB(head_num, dim_in, dim_in, dim_out, use_layer_norm)
    
    def forward(self, X):
        return self.mab(X, X)

class ISAB(nn.Module):
    def __init__(self, head_num, m, dim_in, dim_out, use_layer_norm = True):
        super().__init__()
        self.inducing_point = Parameter(torch.normal(0, 1, (m, dim_in)))
        self.mab1 = MAB(head_num, dim_in, dim_in, dim_out, use_layer_norm)
        self.mab2 = MAB(head_num, dim_out, dim_out, dim_out, use_layer_norm)
